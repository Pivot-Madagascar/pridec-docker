...
If you want to fetch data from the DHIS2 instance and save it locally, there should be a directory named `input` in your current working directory.

If you wish to post forecasts, they must be saved as `output/forecast.json`.

### Environmental Variables

It is easiest to use a `.env` file in your current directory to find the tokens and URLs needed for accessing the API endpoints. It can be provided to the docker image using the `---env-from-file` flag when running.  The `.env` file should follow this template:
...
Individual environmental variables can also be provided to the docker image via the `--env` flag when running.
